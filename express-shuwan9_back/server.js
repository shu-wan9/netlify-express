...
router.get("/api/exercise/getState", async (req, res) => {
  try {
...
router.post("/api/exercise/setState", async (req, res) => {
  try {
...
router.get("/api/exercise/get", (req, res) => {
  const data = db.read();
...
router.post("/api/exercise/remove", (req, res) => {
  const { name } = req.body;
  if (!name) {
    res.sendMsg("任务名必传");
    return;
...
router.post("/api/exercise/clear", async (req, res) => {
  try {
    await clearState();
    res.sendMsg("清除成功");
  } catch (e) {
